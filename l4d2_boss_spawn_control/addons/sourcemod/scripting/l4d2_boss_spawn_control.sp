...
void GetBanFlow(const char[] sMap)
{
	...
	BuildPath(Path_SM, buffer, sizeof(buffer), "%s", g_sMapInfoPath);
	KeyValues kv = new KeyValues("");
	if (!kv.ImportFromFile(buffer))
		SetFailState("Failed to load: %s", buffer);

	FormatEx(buffer, sizeof(buffer), "%s/tank_ban_flow", sMap);
	for (bool iter = kv.JumpToKey(buffer) && kv.GotoFirstSubKey(); iter; iter = kv.GotoNextKey())
	{
