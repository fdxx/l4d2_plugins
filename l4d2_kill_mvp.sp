...
void ShowTotalDamageRank()
{
	...
	if (iCount > 0)
	{
		...
		for (int i; i < iCount; i++)
			CPrintToChatAll("{blue}伤害{default}:  {yellow}%-6i  {blue}特感{default}:  {yellow}%-3i  {blue}丧尸{default}:  {yellow}%-4i  {blue}友伤{default}:  {yellow}%-5i  {blue}|{default}  {olive}%N", data[i].damage, data[i].SI, data[i].CI, data[i].FF, data[i].player);

		if (iTotalDmg > 0)
			CPrintToChatAll("{blue}[MVP]{default}: {olive}%N   {blue}总伤害{default}:  {yellow}%i  {default}({yellow}%i{default}%%)", data[0].player, data[0].damage, RoundToNearest(float(data[0].damage)/iTotalDmg*100));
		
		if (iTotalFF > 0)
		{
			g_iSortField = killData::FF;
			SortCustom2D(data, iCount, SortByDescending);
			CPrintToChatAll("{blue}[LVP]{default}: {olive}%N   {blue}黑枪值{default}:  {yellow}%i  {default}({yellow}%i{default}%%)", data[0].player, data[0].FF, RoundToNearest(float(data[0].FF)/iTotalFF*100));
		}
	}
}
