...
Action CmdListener_vocalize(int client, const char[] command, int argc)
{
	...
	if (!client || !IsClientInGame(client) || GetClientTeam(client) != 2 || !IsPlayerAlive(client) || IsFakeClient(client))
		return Plugin_Continue;

	if (GetCmdArgString(sArg, sizeof(sArg)) < 2 || strncmp(sArg, "smartlook #", 11))
		return Plugin_Continue;
	...
	if (GetEntPropString(entity, Prop_Data, "m_ModelName", sModel, sizeof(sModel)) < 2 || !g_smModelToName.GetString(sModel, sName, sizeof(sName)))
		return Plugin_Continue;
