	...
	if (client > 0 && IsClientInGame(client) && !IsFakeClient(client))
	{
		...
		if (fNewLerp != g_fLastLerp[client])
		{
			if (GetClientTeam(client) > 1)
				CPrintToChatAllEx(client, "{teamcolor}%N {default}@ {teamcolor}%.1f {default}<== {teamcolor}%.1f", client, fNewLerp*1000, g_fLastLerp[client]*1000);
...
Action PlayerTeamChanged_Timer(Handle timer, int userid)
{
	int client = GetClientOfUserId(userid);
	if (client > 0 && IsClientInGame(client) && !IsFakeClient(client))
		CPrintToChatAllEx(client, "{teamcolor}%N {default}@ {teamcolor}%.1f", client, g_fLastLerp[client]*1000);
