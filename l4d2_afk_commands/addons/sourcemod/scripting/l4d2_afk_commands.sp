...
void Init()
{
	...
	LoadTranslations("l4d2_afk_commands.phrases.txt");
}
...
Action GoAfk_CmdListener(int client, const char[] command, int argc)
{
	switch (g_iIdleType)
	{
		case IDLE_BLOCK:
		{
			...
			{
				PrintHintText(client, "%t", "IdleUseAway");
...
Action Cmd_JoinSpectate(int client, int args)
{
	if (IsRealClient(client) && GetClientTeam(client) != 1)
	{
		if (g_fAfkDelay >= 0.1)
		{
			...
			PrintHintText(client, "%t", "Xseconds2spec", g_fAfkDelay);
		}
		else
		{
			ChangeClientTeam(client, 1);
			CPrintToChatAll("%t", "EnterSpectator", client);
		}
	}
	return Plugin_Handled;
}
...
Action Cmd_JoinSurvivor(int client, int args)
{
	if (IsRealClient(client) && GetClientTeam(client) != 2)
	{
		...
		if (bot > 0)
		{
			...
			PrintHintText(client, "%t", "NoBotTakeOver");
	}
	return Plugin_Handled;
}
