...
void LoadAdvertisements()
{
	...
	FindConVar("hostport").GetString(buffer, sizeof(buffer)); // Get config by port
	Format(buffer, sizeof(buffer), "%s/advertisements", buffer);

	for (bool iter = kv.JumpToKey(buffer) && kv.GotoFirstSubKey(false); iter; iter = kv.GotoNextKey(false))
	{
		kv.GetString(NULL_STRING, buffer, sizeof(buffer));
		g_aAdList.PushString(buffer);
	}
